...
import { Row, Col, Input, Table, Card, message, Button, InputNumber } from 'antd';
...
const Sale = () => {
        ...
        setCart(prevCart => {
            ...
            if (existing) {
                return prevCart.map(item => 
                    item.id === product.id 
                    ? { ...item, quantity: item.quantity + 1, import: (item.quantity + 1) * item.price }
                    : item
                );
            }
    ...
    const productColumns = [
        ...
        { title: 'Precio', dataIndex: 'price', key: 'price', render: (p) => `$${p}` },
        { 
            title: 'Acción', 
            key: 'action', 
            render: (_, record) => (
                <Button type="link" onClick={() => addToCart(record)}>Agregar</Button>
            ) 
        },
    ];
    ...
    const cartColumns = [
        ...
        { title: 'Importe', dataIndex: 'import', key: 'import', render: (i) => <strong>${i}</strong> },
        {
            title: '',
            key: 'delete',
            render: (_, record) => (
